...
class Joint(object):
  ...
  def __init__(self, ax: Axes3D, color=None, drawing_enabled: bool = True,
    ...
    Args:
        ...
        drawing_enabled (bool): Whether to create drawing artists. Defaults to True.
        frame_size (float, optional): Length of frame axes for visualization.
    """
  ...
  def pos(self, value: float) -> None:
    """Set joint position and recompute the DH transform.

    Args:
        value (float): joint position in radians
    """
    self._pos = value
    self._theta = value + self._theta_offset
  ...
  def set_dh_parameters(self, a: float, alpha: float, d: float,
                         theta_offset: float = 0.0) -> None:
    """Set all fixed DH parameters and optional theta offset at once.

    Args:
        a (float): link length (mm)
        alpha (float): link twist (rad)
        d (float): link offset (mm)
        theta_offset (float): constant added to joint position when computing
                              the DH transform. Defaults to 0.
    """
    self._a = a
    self._alpha = alpha
    self._d = d
    self._theta_offset = theta_offset
    self._theta = self._pos + theta_offset
    self.update_dh_transform()
  ...
  def set_dh_transform(self, transform: np.ndarray = None):
    """Set the DH transform of the joint.

    Args:
        transform (np.ndarray, optional): (4,4) array of the complete transform.
...
class RobotPayload(object):
  """Draws a marker at the robot payload (end-effector) location."""

  def __init__(self, ax, drawing_enabled=True):
    self._drawing_enabled = drawing_enabled
    if self._drawing_enabled:
      self._point = PointDrawing(ax, color='black')
    else:
      self._point = None
    self._drawn_once = False
    self._enabled = False
  ...
  def set_position(self, pos: np.ndarray):
    """Set the payload position from a (3,) position vector."""
  ...
  def update_frame(self, frame):
    if self._drawing_enabled and self._point is not None:
      self._point.update_frame(frame)

  def draw(self):
    if not self._drawing_enabled or self._point is None:
      return
    if self._frame is not None:
      self._point.update_frame(self._frame)
    if not self._drawn_once:
      self._point.draw()
      self._drawn_once = True
    else:
      self._point.redraw()
    if self._enabled:
      self._point.set_visible()
    else:
      self._point.set_invisible()
