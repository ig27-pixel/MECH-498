...
class FrameDrawing(object):
  ...
  LENGTH = 0.1  # Length of the frame arms, change for frame size
  ...
  def __init__(self, ax: Axes3D, color=None, frame_size=None):
    ...
    Args:
        ...
        frame_size (float, optional): length of frame axes. Defaults to class constant LENGTH.
    """

    self._ax = ax
    self.color = color

    # Allow per-instance frame size (e.g. 75 mm for a mm-scale robot plot)
    length = frame_size if frame_size is not None else self.LENGTH
    self.X_UNIT = np.array([1, 0, 0]) * length
    self.Y_UNIT = np.array([0, 1, 0]) * length
    self.Z_UNIT = np.array([0, 0, 1]) * length
  ...
  def redraw(self):
    """Update artist data; caller is responsible for a single canvas flush."""
...
class PointDrawing(object):
  """Draws a single moving point in 3-D space."""
  ...
  def draw(self):
    (self._point_artist,) = self._ax.plot(
        [self._frame[0, 3]], [self._frame[1, 3]], [self._frame[2, 3]],
        color='black' if self._color is None else self._color,
        marker='o', markersize=10)

  def _update_drawing(self):
    self._point_artist.set_xdata([self._frame[0, 3]])
    self._point_artist.set_ydata([self._frame[1, 3]])
    self._point_artist.set_3d_properties([self._frame[2, 3]])

  def redraw(self):
    self._update_drawing()
    self._point_artist.set_animated(True)
    self._ax.draw_artist(self._point_artist)
    self._point_artist.set_animated(False)

  def set_visible(self):
    if self._point_artist is not None:
      self._point_artist.set_visible(True)

  def set_invisible(self):
    if self._point_artist is not None:
      self._point_artist.set_visible(False)
